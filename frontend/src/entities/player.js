...
import { getAIResponse } from "../content/oldmanDialogue.js"; // Import AI chat function
import { dialog } from "../uiComponents/dialog.js";
import { showPrompt } from "../utils.js"; // ✅ Import custom prompt function
...
export function setPlayerControls(k, player) {
  ...
  k.onKeyPress(async (key) => {
    ...
    if (key === "t") {
      const playerChoice = await showPrompt(k, "Do you want AI dialogue? (yes/no)");

      const aiResponse = await getAIResponse(userInput);
      await dialog(k, player.pos, [aiResponse]); // Display AI chat
    }
  });
